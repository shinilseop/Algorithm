...
public class Ex06_3 {
    ...
    public class Have {
        ...
        public void sellGold(int gold, int price) {
            this.gold -= gold;
            this.money += gold * price;
        }

        public void buyGold(int gold, int price) {
            this.gold += gold;
            this.money -= gold * price;
        }
    }
    ...
    public void buy(String name, int amount, int price, int seq) {
        ...
        for (int i = 0; i < sell.size(); i++) {
            ...
            if (amount > trade.amount) {
                map.get(name).buyGold(trade.amount, Math.min(price, trade.price));
                map.get(trade.name).sellGold(trade.amount, Math.min(price, trade.price));
                amount -= trade.amount;
                trade.amount -= trade.amount;
                removeIdx++;
            } else {
                map.get(name).buyGold(amount, Math.min(price, trade.price));
                map.get(trade.name).sellGold(amount, Math.min(price, trade.price));
    ...
    public void sell(String name, int amount, int price, int seq) {
        ...
        for (int i = 0; i < buy.size(); i++) {
            ...
            if (amount > trade.amount) {
                map.get(name).sellGold(trade.amount, Math.min(price, trade.price));
                map.get(trade.name).buyGold(trade.amount, Math.min(price, trade.price));
                amount -= trade.amount;
                trade.amount -= trade.amount;
                removeIdx++;
            } else {
                map.get(name).sellGold(amount, Math.min(price, trade.price));
                map.get(trade.name).buyGold(amount, Math.min(price, trade.price));
